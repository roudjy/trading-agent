"""Deterministic fail-closed report-lag policy projection."""
...
def _policy_row(row: dict[str, object], policy_row: dict[str, object]) -> dict[str, object]:
    ...
    support_status = str(row.get("report_lag_support") or "unknown").upper()
    ...
    if requirement_status == "NOT_REQUIRED":
        ...
        block_reasons.append("MISSING_REPORT_LAG_POLICY")
    elif support_status == "UNKNOWN":
        policy_status = "POLICY_MISSING"
        block_reasons.extend(["MISSING_REPORT_LAG_POLICY", "REPORT_LAG_UNKNOWN"])
    elif support_status == "UNSUPPORTED":
        policy_status = "UNSUPPORTED"
        block_reasons.extend(["MISSING_REPORT_LAG_POLICY", "REPORT_LAG_UNSUPPORTED"])
    ...
    return {
        ...
        "operator_explanation": (
            "Report-lag policy remains fail-closed until filing latency semantics and review status are explicit."
        ),
    }


def build_report_lag_policy() -> dict[str, object]:
    ...
    return {
        ...
        "summary": {
            ...
            "operator_summary": (
                "Report-lag policy is deterministic and fail-closed. "
                "Manifest presence does not imply usable filing-lag semantics."
            ),
        },
        "rows": rows,
        "safety_invariants": {
