"""Deterministic fail-closed restatement policy projection."""
...
def _policy_row(row: dict[str, object], policy_row: dict[str, object]) -> dict[str, object]:
    ...
    support_status = str(row.get("restatement_history_support") or "unknown").upper()
    ...
    if requirement_status == "NOT_REQUIRED":
        ...
        block_reasons.append("MISSING_RESTATEMENT_POLICY")
    elif support_status == "UNKNOWN":
        policy_status = "POLICY_MISSING"
        block_reasons.extend(["MISSING_RESTATEMENT_POLICY", "RESTATEMENT_POLICY_UNKNOWN"])
    elif support_status == "UNSUPPORTED":
        policy_status = "UNSUPPORTED"
        block_reasons.extend(["MISSING_RESTATEMENT_POLICY", "RESTATEMENT_UNSUPPORTED"])
    ...
    return {
        ...
        "operator_explanation": (
            "Restatement policy remains fail-closed until revision-history semantics and review status are explicit."
        ),
    }


def build_restatement_policy() -> dict[str, object]:
    ...
    return {
        ...
        "report_kind": "restatement_policy",
        ...
        "summary": {
            ...
            "operator_summary": (
                "Restatement policy is deterministic and fail-closed. "
                "Manifest presence does not imply usable restatement semantics."
            ),
        },
        "rows": rows,
        "safety_invariants": {
