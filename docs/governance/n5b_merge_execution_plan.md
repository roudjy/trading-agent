...
| **3 — Operator-confirmed simulator / sacrificial target** | **Module implemented locally (recorded-fixture simulator only); dashboard wiring operator-applied** | The simulator route module at `POST /api/agent-control/merge-execution/simulate` is implemented in [`dashboard/api_merge_execution_simulate.py`](../../dashboard/api_merge_execution_simulate.py), token-gated by N4b (reuses the existing `mobile_approval_dispatch` intent — **no new N4b intent**) and gated by the operator-set `ADE_N5B_SIMULATOR_ENABLED` env flag. The reporting-side projector at [`reporting/n5b_merge_execution_simulate.py`](../../reporting/n5b_merge_execution_simulate.py) reads a closed-schema on-disk fixture (`state/n5b_simulator_fixture.json` by default, gitignored, operator-provided) and writes `phase3_simulation/latest.json` + `phase3_simulation/history.jsonl` under `logs/n5b_merge_execution/`. The selected Phase 3 path is the **recorded-fixture simulator** — the **sacrificial GitHub repository path is rejected** per [`n5b_phase3_implementation_plan.md`](n5b_phase3_implementation_plan.md) §1.4. **No real GitHub call**, no `gh`/`git`/`subprocess`/network — pinned by AST + source-text scans. On `status="ok"` + `would_proceed=true` the endpoint signals *"dry-run checks passed and audit artefacts written"* — NEVER *"merge executed"*, *"PR mutated"*, *"deploy triggered"*, *"live execution authorized"*. The dashboard wiring was applied separately by the operator on the B2.9 branch (B2.0c precedent — Claude is blocked from `dashboard/dashboard.py` by `.claude/hooks/deny_no_touch.py`). | No (recorded-fixture only; no real PR or repo is touched). | Sub-unit operator-go phrases given on each of B2.9a–B2.9e; the `dashboard/dashboard.py` wiring is a distinct operator-applied commit on the same branch. |
| **4 — Production PR merge, if ever approved** | **Not implemented — permanently denied for ADE per §11.** ADE / Claude is NOT the authority that may flip `ADE_N5B_LIVE_EXECUTE_ENABLED=true` on any system. Symmetric to the trading-side rule: ADE never live trades; paper/shadow is the maximum trading-side ADE end state. Phase 3 recorded-fixture simulator is the maximum allowed merge-like ADE surface. | Yes (a single PR per invocation) — were it ever approved by a non-ADE actor. | **Permanently denied for ADE.** A hypothetical Phase 4 would require a separate operator-authored ADR explicitly enabling a non-ADE actor surface as the merge authority. ADE / Claude cannot author such an ADR. |
...
* **No merge without exact operator confirmation.** Each live
  ...
  mint.
* **Phase 4 production PR merge is permanently denied for ADE.**
  Added by B2.9e. ADE / Claude is NOT the authority that may
  flip `ADE_N5B_LIVE_EXECUTE_ENABLED=true` on any system. Even
  with a hypothetical operator-go, the eventual Phase 4
  authority (if it ever exists) must be a non-ADE operator
  surface authored under a separate operator-authored ADR.
  This is symmetric to the trading-side doctrine: live trading
  is permanently denied for ADE; paper/shadow is the maximum
  trading-side ADE end state. Merge-side: Phase 4 is
  permanently denied for ADE; Phase 3 recorded-fixture
  simulator is the maximum allowed merge-like ADE surface.
