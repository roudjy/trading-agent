...
| `research/qre_behavior_thesis_registry.py` | Thesis authority always forbade executable generation and strategy registration. | Prevented thesis surfaces from expressing research-only generation eligibility for complete, approved theses. | Thesis surfaces may expose bounded research-only generation eligibility for ADE-QRE-019, but strategy admission is resolved through the generated registry and canonical resolver outside `research/**`. Candidate promotion, campaign launch without preregistration, and paper/shadow/live authority remain forbidden. | Thesis-registry tests; generation-eligibility tests. |
