...
1. **Generate a VAPID keypair** (one-shot, agent prepares the script
   on operator request; the operator runs it). Write the public key
   text to `config/web_push_vapid_public.txt` (gitignored). Keep the
   private key out of the repo entirely; re-encode it as the env-var
   value (see step 2).
