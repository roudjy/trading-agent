...
| **v3.15.15.20** | **Approval / exception inbox** (`reporting.approval_inbox` + `dashboard.api_approval_inbox` + Inbox card on the PWA). Read-only aggregation of every needs_human / blocked / high-risk / unknown item from the upstream reporters, projected into 18 canonical categories. No approve / reject / execute buttons. See [`approval_exception_inbox.md`](approval_exception_inbox.md). |
| **v3.15.15.21** | **Execute-safe controls + approved read-only PWA route activation** (`reporting.execute_safe_controls` + `dashboard.api_execute_safe_controls` + Execute-safe card on the PWA). Closed catalog of four typed actions; HIGH-risk never eligible; frozen-contract drift detection; CLI-only execution (no POST endpoint). The four-line `dashboard.py` wiring authorized by the operator activates all v3.15.15.18 / .19 / .20 / .21 read-only routes in production. See [`execute_safe_controls.md`](execute_safe_controls.md). |
