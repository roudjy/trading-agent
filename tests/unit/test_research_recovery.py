...
def test_resume_with_retry_failed_batches_reuses_completed_batches_and_creates_new_run_id(monkeypatch, tmp_path: Path):
    _patch_common_runner(monkeypatch, tmp_path, _RetryableFailureEngine)
    ...
    failed_state = _load_json(tmp_path / "research" / "run_state.v1.json")
    failed_batches = _load_json(tmp_path / "research" / "run_batches_latest.v1.json")
    ...
    resumed_state = _load_json(tmp_path / "research" / "run_state.v1.json")
    resumed_manifest = _load_json(tmp_path / "research" / "run_manifest_latest.v1.json")
    resumed_batches = _load_json(tmp_path / "research" / "run_batches_latest.v1.json")
...
def test_resume_recovers_stale_running_validation_batch_with_new_run_id(monkeypatch, tmp_path: Path):
    _patch_common_runner(monkeypatch, tmp_path, _InterruptibleValidationEngine)
    _set_multi_batch_strategies(monkeypatch)
    ...
    stale_state = _load_json(tmp_path / "research" / "run_state.v1.json")
    stale_batches = _load_json(tmp_path / "research" / "run_batches_latest.v1.json")
