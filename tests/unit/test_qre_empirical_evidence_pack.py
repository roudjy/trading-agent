...
def test_build_empirical_evidence_pack_is_canonical_and_fail_closed(tmp_path: Path) -> None:
    ...
    assert payload["transaction_costs"]["outcome"] == "INCONCLUSIVE"
    assert payload["oos"]["sufficiency"] == "INSUFFICIENT"
    assert payload["evidence_semantics"]["oos"]["presence"] == "AVAILABLE"
    assert payload["decision_semantics"]["active_blocker"] == "REQUEST_MORE_EVIDENCE"
